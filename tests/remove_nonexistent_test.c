/**
 * @file remove_nonexistent_test.c
 * @author Carl Mattatall (cmattatall2@gmail.com)
 * @brief Source module to test the API behaviour when caller attempts to
 * remove elements that are not in the table
 * @version 0.1
 * @date 2021-01-11
 *
...
int main(void)
{
    ...
    for (i = 0; i < max_i; i++)
    {
        /* RETVAL SHOULD INDICATE ERROR (item not in table) */
        if (0 == htbl_remove(tbl, NULL))
        {
            return 1;
        }
    }
